...
namespace Contabilizacao.ExternalConnections.GoogleSheets
{
    public static class SpreadsheetRangeHelper
    {
        ...
        public static string GetUpdateRequestRange(int row, int firstColumn, int lastColumn, string page)
        {
            char charFirstColumn = (char)(firstColumn + 'A' - 1);
            char charLastColumn = (char)(lastColumn + 'A' - 1);

            return page + "!" + charFirstColumn.ToString() + row.ToString() + ":" + charLastColumn.ToString() + row.ToString();
        }
    }
}
